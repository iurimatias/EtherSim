...
var optionalCallback = function (cb) {
...
Manager.prototype.request = function(params, cb) {
  ...
  if (cb) {
    ...
    args.push(function (err, res) {
      cb(err, that.response(params, res));
    });
  }
...
Manager.prototype.eth_getBlockByHash = function(tx_hash, include_transactions, callback) {
  callback = optionalCallback(callback);
  return callback(null, this.blockchain.getBlockByHash(tx_hash));
};
...
Manager.prototype.eth_getTransactionCount = function(address, block_number, callback) {
  callback = optionalCallback(callback);
  return callback(null, this.blockchain.getTransactionCount(address, callback));
}

Manager.prototype.eth_sendTransaction = function(tx_data, callback) {
  callback = optionalCallback(callback);
  return callback(this.blockchain.queueTransaction(tx_data, callback));
};
